...
class ConfluenceIngestion:
    ...
    def fetch_children(self, page_id):
        ...
        if response.status_code == 200:
            ...
            return [child["title"] for child in children_data.get("results", [])]
        else:
            logger.warning(f"Could not fetch children for page {page_id}")
            return []

    def extract_confluence_data(self):
            ...
            if response.status_code == 200:
                ...
                for page in pages.get("results", []):
                    ...
                    page_data = {
                        ...
                        "ancestors": [
                            ancestor["title"] for ancestor in page.get("ancestors", [])
                        ],
    ...
    def save_to_json(self, data, filename="data/confluence_data.json"):
    ...
    def extract_and_process_confluence_data(self):
